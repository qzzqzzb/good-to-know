...
USER_CONTEXT_PATH = SKILL_DIR / "user_context.md"
...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("Usage: python3 memory/naive-memory/scripts/ingest_context.py <context_outbox_path>")
    ...
    user_context_text = USER_CONTEXT_PATH.read_text(encoding="utf-8") if USER_CONTEXT_PATH.exists() else "# User Context Memory\n\n"
    known_entry_ids = existing_entry_ids(user_context_text)
    known_dedup_keys = existing_dedup_keys(user_context_text)
    ...
    if not new_blocks:
        print("[naive-memory] imported 0 new entry(s)")
        return

    suffix = "\n\n".join(new_blocks).strip() + "\n"
    base = user_context_text.rstrip() + "\n\n"
    USER_CONTEXT_PATH.write_text(base + suffix, encoding="utf-8")
    print(f"[naive-memory] imported {len(new_blocks)} new entry(s) from {outbox_path}")
