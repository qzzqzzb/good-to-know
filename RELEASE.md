...
6. Do a clean install smoke test in a temporary environment:
   ...
   tmpdir=$(mktemp -d)
   uv venv "$tmpdir/.venv"
   uv pip install --python "$tmpdir/.venv/bin/python" dist/goodtoknow_gtn-0.2.0-py3-none-any.whl
...
7. Do one install-gtn-style upgrade smoke test against a disposable `GTN_HOME`:
   ...
   uv pip install --python "$GTN_HOME/.venv/bin/python" --upgrade --force-reinstall dist/goodtoknow_gtn-0.2.0-py3-none-any.whl
...
```bash
uv publish dist/goodtoknow_gtn-0.2.0.tar.gz dist/goodtoknow_gtn-0.2.0-py3-none-any.whl
```
...
1. Install from PyPI in a clean environment:
   ...
   tmpdir=$(mktemp -d)
   uv venv "$tmpdir/.venv"
   uv pip install --python "$tmpdir/.venv/bin/python" goodtoknow-gtn==0.2.0
   "$tmpdir/.venv/bin/gtn" --help
   ```
...
2. Validate the install path used by `install_gtn`:
   ...
   ```bash
   uv pip install --python ~/.gtn/.venv/bin/python --upgrade goodtoknow-gtn==0.2.0
   env -u NO_COLOR gtn status
   ```
